...
export default {
  async fetch(request, env) {
    ...
    if (path === "/api/login" && method === "POST") {
      ...
      const match = await bcrypt.compare(password, row.pw_hash);
      if (!match) return err("Invalid username or password", 401, origin);
