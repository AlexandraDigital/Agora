...
// All admin checks use the isAdmin() helper (reads the `isAdmin` DB column)
// instead of the old ADMIN_USERS username array. The old code compared
// cu.id (a user-id string) against username strings, so admin routes always
// returned 403 even for the real admin account.
    ...
    result.results.forEach(row => {
      if (!flagsMap[row.postId]) {
        flagsMap[row.postId] = {
          ...
          author: { id: row.authorId, username: row.username, avatar: row.avatar },
