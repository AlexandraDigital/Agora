...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (securityQuestion && !securityAnswer) {
      return errResponse("Please provide an answer for your security question", 400);
    }
    ...
    const secAnswerHash = securityAnswer ? await hashPassword(securityAnswer.trim().toLowerCase()) : null;
    ...
    .bind(username, displayName, bio || "", pw_hash, initials, avatarColor, avatarStyle, rightNow, securityQuestion?.trim() || null, secAnswerHash)
    .run();
