...
function AvatarCustomizer({ user, onSave, onCancel }) {
  const [tab, setTab] = useState(user.avatarImage && !user.avatarImage.startsWith("data:image/png") ? "upload" : "emoji");
  ...
  const [uploadedImage, setUploadedImage] = useState(user.avatarImage && tab === "upload" ? user.avatarImage : null);
  ...
  const handleImageFile = async (e) => {
    ...
    if (compressed) setUploadedImage(`data:image/jpeg;base64,${compressed}`);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (tab === "upload" && uploadedImage) {
        ...
        onSave({ avatarImage: png, avatar: selectedEmoji, avatarColor: bgColor, avatarStyle: avatarShape });
      }
    } finally {
      setSaving(false);
    }
...
export default function Agora() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg }}>
      ...
      {editingAvatar && <AvatarCustomizer user={cu} onSave={handleSaveAvatar} onCancel={()=>setEditingAvatar(false)}/>}
