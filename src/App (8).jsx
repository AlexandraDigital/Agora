...
const hasId = (ids = [], id) => ids.map(String).includes(String(id));
...
const ALLOWED_TYPES = {
  ...
  "audio/mpeg":  { sig: [[0x49,0x44,0x33]], sigAlt: [[0xFF,0xFB]], ext: ["mp3"] }, // ID3 tag, or a bare frame sync with no tag
  "audio/mp4":   { sig: null, ext: ["m4a"] }, // MPEG-4 container — skip magic check, like video/mp4 above
  "audio/webm":  { sig: [[0x1A,0x45,0xDF,0xA3]], ext: ["webm"] }, // same EBML header as video/webm
  "audio/ogg":   { sig: [[0x4F,0x67,0x67,0x53]], ext: ["ogg","oga"] }, // same Ogg header as video/ogg
  "audio/wav":   { sig: null, ext: ["wav"] }, // RIFF container — skip magic check, like image/webp above
};
...
const moderateMedia = async (file) => {
  try {
    ...
    if (!meta) return { ok: false, reason: `File type "${file.type}" is not allowed. Upload JPEG, PNG, GIF, WebP, MP4, WebM, MOV, MP3, M4A, WAV, or OGG.` };
    ...
    if (meta.sig || meta.sigAlt) {
      const header = await readHeader(file);
      if (!header) return { ok: false, reason: "Could not read the file. Try another." };
      const okSig = meta.sig && matchesSig(header, meta.sig);
      const okAlt = meta.sigAlt && matchesSig(header, meta.sigAlt);
      if (!okSig && !okAlt) return { ok: false, reason: "File header doesn't match its declared format. The file may be corrupted or mislabeled." };
    }
...
function PostCard({ post, users, cu, token, onLike, onComment, onCommentReply, onDelete, onDeleteComment, onUser, onError, onToast, onEdit, hideCounts }) {
  ...
  const [discussionPrompt, setDiscussionPrompt] = useState(
    post.discussionPrompt || generateDiscussionPrompt(post.content, post.media?.type)
  );
      ...
      {post.media && (
        <div style={{ marginBottom:2, overflow:"hidden" }}>
          {post.media.type === "video" && post.media.videoUrl
            ...
            : post.media.type === "audio" && post.media.videoUrl
            ? (
              <div style={{ padding:"14px 16px", background:C.accentLight, display:"flex", alignItems:"center", gap:12 }}>
                <span style={{ fontSize:20, flexShrink:0 }}>🎵</span>
                <audio src={post.media.videoUrl} controls style={{ width:"100%", height:36 }}/>
              </div>
            )
  ...
  <DiscussionPrompt postText={post.content} mediaType={post.media?.type} postId={post.id} token={token} initialPrompt={discussionPrompt} onPromptChange={setDiscussionPrompt} /> 
</div> 
...
function ProfileScreen({ uid, users, cu, token, onFollow, onBack, onLike, onComment, onCommentReply, onDelete, onDeleteComment, onUser, onError, onEditAvatar, onToast, onEdit, hideCounts, onMessage }) {
  const user = users.find(u=>sameId(u.id, uid));
  const isOwn = sameId(uid, cu.id);
  const following = hasId(cu.following, uid);
  const [followBusy, setFollowBusy] = useState(false);
  // onFollow (the `follow` handler in the parent) is fire-and-forget from
  // this button's point of view — it doesn't return anything we can await
  // to know when the toggle has actually landed. A fast double-tap here
  // fires two overlapping /api/follow/:id POSTs, which flips the DB row
  // twice and leaves this button showing the opposite of the true server
  // state (the exact case the "Follow status may not be up to date" toast
  // in the parent's follow() catch block exists to paper over). Guard the
  // button itself instead: block re-entry until the current toggle's
  // local state update has applied, the same pattern already used below
  // for Mute/Block via modBusy.
  ...
  // Tracks whether the current isBlocked/isMuted values came from a click
  // in THIS mount, so the status-loading effect below can tell "stale data
  // for a new profile" (should overwrite) apart from "the effect re-ran
  // after the user already toggled something" (should NOT overwrite —
  // that was the actual bug: a parent re-render was re-running this effect
  // with the same uid/token and stomping the optimistic click with
  // whatever it re-fetched, which could itself be racy against the POST
  // that hadn't committed yet).
  ...
  const fetchModStatus = async () => {
    ...
    return {
      blocked: Array.isArray(blocked) && blocked.map(String).includes(String(uid)),
      muted:   Array.isArray(muted)   && muted.map(String).includes(String(uid)),
    };
  };

  useEffect(() => {
    if (isOwn) return;
    localOverride.current = false; // new profile (or first mount) — trust the server
    fetchModStatus().then(({ blocked, muted }) => {
      if (localOverride.current) return; // a click already landed while this was in flight
  ...
  const toggleBlock = async () => {
    ...
    const action = prev ? "unblock" : "block";
    localOverride.current = true;
    setIsBlocked(!prev); // optimistic, so the tap feels instant
    try {
      const res = await fetch(`/api/users/${uid}/${action}`, { method:"POST", headers:{ Authorization:`Bearer ${token}` }});
      const data = await res.json().catch(()=>({}));
      if (!res.ok || data?.error) throw new Error(data?.error || `Request failed (${res.status})`);
      // Confirm against the server rather than trusting the optimistic flip —
      // this is what actually catches "it said Unblock but reverted": if the
      // write didn't really stick, this shows the true state instead of a
      // button that silently snaps back with no explanation.
      const { blocked } = await fetchModStatus();
      setIsBlocked(blocked);
      onToast?.({ message: blocked ? "User blocked." : "User unblocked.", type:"success" });
    } catch {
      setIsBlocked(prev); // roll back the optimistic flip
      onToast?.({ message:"Action failed. Please try again.", type:"error" });
    }
    setModBusy(false);
  };
  ...
  const toggleMute = async () => {
    ...
    setModBusy(true);
    const prev = isMuted;
    const action = prev ? "unmute" : "mute";
    localOverride.current = true;
    setIsMuted(!prev); // optimistic, so the tap feels instant
    try {
      const res = await fetch(`/api/users/${uid}/${action}`, { method:"POST", headers:{ Authorization:`Bearer ${token}` }});
      const data = await res.json().catch(()=>({}));
      if (!res.ok || data?.error) throw new Error(data?.error || `Request failed (${res.status})`);
      // Same confirm-against-server step as toggleBlock above.
      const { muted } = await fetchModStatus();
      setIsMuted(muted);
      onToast?.({ message: muted ? "User muted." : "User unmuted.", type:"success" });
    } catch {
      setIsMuted(prev); // roll back the optimistic flip
      onToast?.({ message:"Action failed. Please try again.", type:"error" });
    }
    setModBusy(false);
  };
          ...
          {!isOwn && (
            <div style={{ display:"flex", flexDirection:"column", gap:8, alignItems:"flex-end", flexShrink:0 }}>
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={handleFollowClick} disabled={followBusy} style={{ background:following?"none":C.accent, color:following?C.textMuted:"#fff", border:`1px solid ${following?C.borderStrong:C.accent}`, borderRadius:20, padding:"8px 20px", fontSize:13, cursor:followBusy?"default":"pointer", fontFamily:T.body, fontWeight:500, opacity:followBusy?0.6:1 }}>{following?"Unfollow":"Follow"}</button>
                {!isBlocked && <button onClick={()=>onMessage?.(user)} style={{ background:"none", color:C.accent, border:`1px solid ${C.accent}`, borderRadius:20, padding:"8px 16px", fontSize:13, cursor:"pointer", fontFamily:T.body, fontWeight:500 }}>Message</button>}
              </div>
              <div style={{ display:"flex", gap:6 }}>
...
// Polling interval for the inbox list and an open thread. No websocket/
// Durable Object exists in this codebase for push delivery, so this
// mirrors the rest of the app's fetch-and-refetch approach rather than
// introducing new realtime infrastructure.
const CHAT_POLL_MS = 5000;

function ChatListScreen({ cu, token, onOpenThread, onBack, onError }) {
  const [threads, setThreads] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    try {
      const res = await api.get("/api/threads", token);
  ...
  useEffect(() => {
    load();
    const iv = setInterval(load, CHAT_POLL_MS);
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", cursor:"pointer", color:C.textMuted, fontSize:14, padding:"0 0 16px", fontFamily:T.body, display:"flex", alignItems:"center", gap:4 }}>← back</button>
      <div style={{ fontWeight:700, fontSize:20, fontFamily:T.brand, marginBottom:16 }}>Messages</div>

      {loading && <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:32, textAlign:"center", color:C.textMuted, fontFamily:T.body, fontSize:14 }}>Loading…</div>}
      ...
      {!loading && threads.map(t => (
        <button key={t.id} onClick={()=>onOpenThread(t.id, t.user)} style={{ display:"flex", alignItems:"center", gap:12, width:"100%", textAlign:"left", background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:14, marginBottom:10, cursor:"pointer" }}>
          <Av user={t.user} size={44}/>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", gap:8 }}>
              <div style={{ fontWeight:600, fontSize:14, fontFamily:T.body, color:C.text }}>{t.user.displayName}</div>
              {t.lastMessage && <div style={{ fontSize:11, color:C.textMuted, fontFamily:T.body, flexShrink:0 }}>{fmtTime(t.lastMessage.timestamp)}</div>}
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:2 }}>
              <div style={{ fontSize:13, color:t.unreadCount>0?C.text:C.textMuted, fontFamily:T.body, fontWeight:t.unreadCount>0?600:400, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", flex:1 }}>
                {t.lastMessage ? (sameId(t.lastMessage.senderId, cu.id) ? "You: " : "") + t.lastMessage.content : (t.user.presenceNote || "No messages yet")}
              </div>
              {t.unreadCount>0 && <div style={{ background:C.accent, color:"#fff", borderRadius:10, fontSize:11, fontWeight:700, minWidth:18, height:18, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 5px", flexShrink:0 }}>{t.unreadCount}</div>}
            </div>
          </div>
        </button>
      ))}
...
function ThreadScreen({ threadId, otherUser, cu, token, onBack, onError, onToast }) {
  ...
  const load = async ({ silent } = {}) => {
    try {
      // /api/threads/:id (not .../messages) — matches the existing
      // functions/api/threads/[id].js handler, which loads history AND
      // marks the other person's messages read in one GET.
      const res = await api.get(`/api/threads/${threadId}`, token);
  ...
  useEffect(() => {
    load();
    const iv = setInterval(() => load({ silent: true }), CHAT_POLL_MS);
    return () => clearInterval(iv);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [threadId, token]);

  useEffect(() => {
    // Only auto-scroll on the very first load and when the message count
    // grows — not on every silent poll, so scrolling up to re-read older
    // messages doesn't keep getting yanked back to the bottom.
  ...
  const send = async () => {
    ...
    setDraft("");
    try {
      // Same endpoint as load() — [id].js handles both GET (history) and
      // POST (send) on /api/threads/:id. Sent messages default to the
      // backend's 72h vanishing window; there's no pin toggle in this UI
      // yet, so `pin` is left unset here.
      const msg = await api.post(`/api/threads/${threadId}`, { content }, token);
      setMessages(prev => [...prev, msg]);
      bottomRef.current?.scrollIntoView({ block:"end", behavior:"smooth" });
    } catch (e) {
      setDraft(content); // restore so the person doesn't lose what they typed
      onToast?.({ message: e?.message || "Message failed to send.", type:"error" });
    }
    setSending(false);
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 190px)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:14, borderBottom:`1px solid ${C.border}`, marginBottom:14, flexShrink:0 }}>
        <button onClick={onBack} style={{ background:"none", border:"none", cursor:"pointer", color:C.textMuted, fontSize:20, padding:0, display:"flex" }}>←</button>
        <Av user={otherUser} size={34}/>
        <div>
          <div style={{ fontWeight:600, fontSize:14, fontFamily:T.body, color:C.text }}>{otherUser.displayName}</div>
          {otherUser.presenceNote && <div style={{ fontSize:11, color:C.textMuted, fontFamily:T.body }}>{otherUser.presenceNote}</div>}
        </div>
      </div>

      <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:8, paddingRight:2 }}>
        {loading && <div style={{ textAlign:"center", color:C.textMuted, fontFamily:T.body, fontSize:13, padding:20 }}>Loading…</div>}
        {!loading && messages.length===0 && <div style={{ textAlign:"center", color:C.textMuted, fontFamily:T.body, fontSize:13, padding:20 }}>Say hello 👋</div>}
        {messages.map(m => {
          const mine = sameId(m.senderId, cu.id);
          return (
            <div key={m.id} style={{ display:"flex", justifyContent:mine?"flex-end":"flex-start" }}>
              <div style={{ maxWidth:"75%", background:mine?C.accent:C.surface, color:mine?"#fff":C.text, border:mine?"none":`1px solid ${C.border}`, borderRadius:16, borderBottomRightRadius:mine?4:16, borderBottomLeftRadius:mine?16:4, padding:"9px 13px", fontSize:14, fontFamily:T.body, lineHeight:1.4, wordBreak:"break-word" }}>
                {m.content}
                <div style={{ fontSize:10, marginTop:3, opacity:0.7, textAlign:"right" }}>{fmtTime(m.timestamp)}</div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display:"flex", gap:8, paddingTop:12, flexShrink:0 }}>
        <input
          ...
          placeholder="Message…"
          maxLength={2000}
          style={{ flex:1, padding:"10px 14px", borderRadius:20, border:`1px solid ${C.border}`, fontSize:14, fontFamily:T.body, background:C.bg, color:C.text, outline:"none" }}
        />
        <button onClick={send} disabled={sending||!draft.trim()} style={{ background:C.accent, color:"#fff", border:"none", borderRadius:20, padding:"0 18px", fontSize:14, fontFamily:T.body, fontWeight:600, cursor:(sending||!draft.trim())?"default":"pointer", opacity:(sending||!draft.trim())?0.6:1 }}>Send</button>
      </div>
    </div>
  );
}
...
function SettingsScreen({ cu, token, users, onLogout, onBack, onUpdate, onToast, hideCounts, onToggleHideCounts, todayMinutes, todaySessions }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!bRes.ok || !mRes.ok) {
          const failed = !bRes.ok ? bRes : mRes;
          let msg = `Failed to load (status ${failed.status})`;
          try {
            const errBody = await failed.clone().json();
  ...
  const unblock = async (uid) => {
    ...
    try {
      const res = await fetch(`/api/users/${uid}/unblock`, { method:"POST", headers:{ Authorization:`Bearer ${token}` }});
      const data = await res.json().catch(()=>({}));
      if (!res.ok || data?.error) throw new Error(data?.error || `Request failed (${res.status})`);
      // Compare as strings — the list came back from the API as raw
      // targetUserId values, which may not be the same type as the uid
      // this handler was called with, so a strict !== could silently
      // leave the row in place even though the unblock succeeded.
      setBlockedUsers(prev => prev.filter(id => String(id) !== String(uid)));
      onToast?.({ message:"User unblocked.", type:"success" });
    } catch {
      onToast?.({ message:"Couldn't unblock. Please try again.", type:"error" });
    }
    setActionBusy(null);
  };
  ...
  const unmute = async (uid) => {
    ...
    try {
      const res = await fetch(`/api/users/${uid}/unmute`, { method:"POST", headers:{ Authorization:`Bearer ${token}` }});
      const data = await res.json().catch(()=>({}));
      if (!res.ok || data?.error) throw new Error(data?.error || `Request failed (${res.status})`);
      setMutedUsers(prev => prev.filter(id => String(id) !== String(uid)));
      onToast?.({ message:"User unmuted.", type:"success" });
    } catch {
      onToast?.({ message:"Couldn't unmute. Please try again.", type:"error" });
    }
    setActionBusy(null);
  };
  ...
  const listCard = (title, emoji, items, onAction, actionLabel, busyPrefix) => (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:20, marginBottom:16 }}>
      ...
      {loadingLists ? (
        ...
        <div style={{ color:"#d63031", fontSize:13, fontFamily:T.body }}>{listError}</div>
      ) : items.length === 0 ? (
...
function EditPostModal({ post, cu, token, onSave, onCancel, onToast }) {
  ...
  const handleReplaceMedia = async (e) => {
    ...
    if (isImg) {
      ...
      setNewMedia({ type: "audio", videoUrl: blobUrl, file: f });
    } else {
      const blobUrl = URL.createObjectURL(f);
  ...
  const doSave = async () => {
    ...
    setSaving(true);
    try {
      let media = newMedia ? { type: newMedia.type, thumb: newMedia.thumb, videoUrl: newMedia.videoUrl } : undefined;
      // newMedia.videoUrl (when set by handleReplaceMedia, above) is a
      // client-local blob: URL from URL.createObjectURL — it only resolves
      // in this tab and would leave the post with a dead link the moment
      // the tab closes. Upload the raw file to KV first and swap in the
      // real /api/video or /api/audio URL, same as ComposeModal does for a
      // new post — replacing media should upload it exactly the same way
      // adding it does.
      if (newMedia?.file && (newMedia.type === "video" || newMedia.type === "audio")) {
        const reader = new FileReader();
        const base64 = await new Promise((res, rej) => {
          reader.onload = e => res(e.target.result.split(",")[1]);
          reader.onerror = rej;
          reader.readAsDataURL(newMedia.file);
        });
        const endpoint = newMedia.type === "video" ? "/api/video" : "/api/audio";
        const resp = await fetch(endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ base64, contentType: newMedia.file.type, size: newMedia.file.size }),
        });
        const data = await resp.json();
        if (!resp.ok) throw new Error(data.error || `${newMedia.type === "video" ? "Video" : "Audio"} upload failed (${resp.status})`);
        media = { type: newMedia.type, thumb: newMedia.thumb, videoUrl: data.url };
      }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(20,34,46,0.6)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:100 }}>
      <div style={{ background:C.surface, width:"100%", maxWidth:600, borderRadius:"20px 20px 0 0", padding:24, maxHeight:"88vh", overflow:"auto" }}>
              ...
              <>
                {newMedia ? (
                  newMedia.type === "video" ? (
                    ...
                    <div style={{ padding:"14px 16px", background:"#faf9f6", display:"flex", alignItems:"center", gap:12 }}>
                      <span style={{ fontSize:20, flexShrink:0 }}>🎵</span>
                      <audio src={newMedia.videoUrl} controls style={{ width:"100%", height:36 }}/>
                    </div>
                  ) : (
        ...
        <input ref={fileRef} type="file" accept="image/*,video/mp4,video/webm,video/quicktime,audio/mpeg,audio/mp4,audio/webm,audio/ogg,audio/wav" onChange={handleReplaceMedia} style={{ display:"none" }}/>
...
function ComposeModal({ cu, token, onPost, onClose }) {
  ...
  const handleFile = async (e) => {
    ...
    if (isAud && f.size > MAX_AUDIO) { setModStatus("error"); setModReason("Audio too large. Max 15MB."); return; }
    ...
    if (isImg) {
      ...
      // Audio has no visual thumbnail to extract or moderate — same
      // file-format checks as image/video (allowlist + extension + magic
      // bytes), just no frame/compression step.
      const result = await moderateMedia(f);
      if (result.ok) {
        setFile({ blobUrl, type: "audio", mime: f.type, raw: f });
        setModStatus("ok");
      } else {
        setModStatus("rejected"); setModReason(result.reason);
        URL.revokeObjectURL(blobUrl);
    ...
    try {
      ...
      if (file) {
        if (file.type === "video") {
          ...
          // Upload audio to KV, same pattern as video above. Reuses the
          // videoUrl field to carry the playable URL — the DB column and
          // shapePost already pass this through generically regardless of
          // media type, so no schema change is needed.
          const reader = new FileReader();
          const base64 = await new Promise((res, rej) => {
            reader.onload = e => res(e.target.result.split(",")[1]);
            reader.onerror = rej;
          ...
          const data = await resp.json();
          if (!resp.ok) { alert("Audio upload failed: " + (data.error || resp.status)); setUploading(false); return; }
          mediaPayload = { type: "audio", videoUrl: data.url };
        } else {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(20,34,46,0.6)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:100 }}>
      <div style={{ background:C.surface, width:"100%", maxWidth:600, borderRadius:"20px 20px 0 0", padding:24, maxHeight:"88vh", overflow:"auto" }}>
          ...
          <div style={{ margin:"14px 0 4px", position:"relative", borderRadius:10, overflow:"hidden", border:`1px solid ${C.border}` }}>
            {file.type === "video"
              ...
              ? (
                <div style={{ padding:"14px 16px", background:"#faf9f6", display:"flex", alignItems:"center", gap:12 }}>
                  <span style={{ fontSize:20, flexShrink:0 }}>🎵</span>
                  <audio src={file.blobUrl} controls style={{ width:"100%", height:36 }}/>
                </div>
              )
            ...
            <div style={{ position:"absolute", top:8, left:8, background:"rgba(0,0,0,0.55)", color:"#fff", fontSize:11, padding:"3px 9px", borderRadius:20, fontFamily:T.mono, display:"flex", alignItems:"center", gap:5 }}>
              <span style={{ color:"#6ddb9a" }}>✓</span> {file.type === "video" ? "video ok" : file.type === "audio" ? "audio ok" : "image ok"}
            </div>
        ...
        <input ref={fileRef} type="file" accept="image/*,video/mp4,video/webm,video/quicktime,audio/mpeg,audio/mp4,audio/webm,audio/ogg,audio/wav" onChange={handleFile} style={{ display:"none" }}/>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:16, paddingTop:16, borderTop:`1px solid ${C.border}` }}>
          ...
          >
            ...
            {file ? "Replace media" : "Add photo / video / audio"}
          </button>
...
export default function Agora() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!us.error) {
        setUsers(us);
        // `cu` may have been hydrated from a stale localStorage snapshot
        // (or drifted after a follow request whose response never made it
        // back — see the catch in `follow` below). `us` is a fresh,
        // DB-backed shapeUser() result for every account including our
        // own, so reconcile `cu.following`/`followers` against it here
        // rather than trusting whatever was cached at login time.
        const fresh = us.find(u => sameId(u.id, cu.id));
        if (fresh) {
          setCu(prev => {
            const merged = { ...prev, following: fresh.following, followers: fresh.followers };
...
const saveSecurityQuestion = async () => {
  ...
  try {
   const res = await api.post(`/api/follow/${Number(uid)}`, {}, token);
    ...
    // The POST may have actually landed server-side even though we
    // failed to read its response (dropped connection, etc.), which is
    // exactly the kind of drift this toast used to just warn about
    // without fixing. Re-pull our own fresh shapeUser() record so the
    // button self-corrects instead of staying wrong until a manual
    // page reload.
    try {
      const fresh = await api.get(`/api/users/${Number(cu.id)}`, token);
      if (!fresh.error) {
        setCu(prev => {
          const merged = { ...prev, following: fresh.following, followers: fresh.followers };
          localStorage.setItem("ag_cu", JSON.stringify(merged));
        ...
        setUsers(prev => prev.map(u => sameId(u.id, uid)
          ? { ...u, followers: hasId(fresh.following, uid)
              ? [...(u.followers || []).filter(id => !sameId(id, cu.id)), Number(cu.id)]
              : (u.followers || []).filter(id => !sameId(id, cu.id)) }
          : u));
      }
    } catch (_) { /* best effort — fall through to the toast either way */ }
...
const doCommentReply = async (pid, text, parentCommentId, quotedCommentId = null) => {
  ...
  // Set when opening a specific conversation (from ChatListScreen or the
  // ProfileScreen "Message" button) so ThreadScreen has who it's talking
  // to without an extra users-array lookup keyed only by id.
  const [activeThread, setActiveThread] = useState(null); // { id, user }

  const openThread = (threadId, otherUser) => {
    setActiveThread({ id: threadId, user: otherUser });
    setScreen("thread");
  };

  const startThreadWithUser = async (otherUser) => {
    try {
      const thread = await api.post("/api/threads", { targetId: otherUser.id }, token);
      openThread(thread.id, otherUser);
    } catch (e) {
      setToast({ message: e?.message || "Couldn't start conversation.", type:"error" });
    }
  };
  ...
  />
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:C.surface, borderTop:`1px solid ${C.border}`, display:"flex", padding:"8px 0 18px", zIndex:50 }}>
        {navItems.filter(item=>!item.adminOnly||cu.isAdmin).map(item=>{
          const active=screen===item.id||(item.id==="myprofile"&&screen==="profile"&&profileUid===cu.id)||(item.id==="chats"&&screen==="thread");
          return (
