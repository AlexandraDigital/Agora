...
function AvatarCustomizer({ user, token, onSave, onCancel }) {
  ...
  const handleImageFile = async (e) => {
    ...
    const dataUrl = `data:image/jpeg;base64,${compressed}`;
    // Store directly in KV via API
    ...
    try {
      ...
      else alert("Upload error: no URL returned");
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (tab === "upload" && uploadedImage) {
        ...
        if (!res.ok) { alert("Failed to save avatar: " + res.status); return; }
        const data = await res.json();
        if (!data.url) { alert("No URL returned from avatar upload"); return; }
        onSave({ avatarImage: data.url, avatar: selectedEmoji, avatarColor: bgColor, avatarStyle: avatarShape });
      }
    } catch (err) {
...
function ComposeModal({ cu, onPost, onClose }) {
  ...
  const handleFile = async (e) => {
    ...
    if (result.ok) {
      setFile({ blobUrl, type: "image", thumb, mime: f.type });
      setModStatus("ok");
    } else {
  ...
  const doPost = () => {
    if (!canPost) return;
    onPost(text.trim(), file ? { type: file.type, thumb: file.thumb, blobUrl: file.blobUrl } : null, videoEmbed ? videoUrl.trim() : null);
    onClose();
  };
...
export default function Agora() {
  ...
  const handleSaveAvatar = async (data) => {
    const res = await api.put(`/api/users/${cu.id}`, data, token);
    if (res.error) {
      alert("Failed to save avatar: " + (res.error || "Unknown error"));
      return;
    }
    setCu(res);
    localStorage.setItem("ag_cu", JSON.stringify(res));
